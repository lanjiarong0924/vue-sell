...
<div class="accessory-list-wrap">
            <div class="accessory-list col-4">
             <h1 class="cooperation">合作的商家</h1>
              <ul>
                <li v-for="(item,index) in bannerArr" :key="index">
                  <div class="pic">
                    <a href="#">
                      <img :src="`${item.productImg}`" alt="" style="height:100%">
                    </a>
                  </div>
                  <div class="main">
                    ...
                    <div class="btn-area">
                    <a href="javascript:;" class="btn btn--m" style="color:tan">{{item.producthpone}}</a>
                      <a href="javascript:;" class="btn btn--m" style="color:black">收藏店铺</a>
                    </div>
                  </div>
                </li>
              </ul>
  ...
  export default {
    ...
    created() {
      this.favorite = loadFromLocal(this.seller.id, 'favorite', false)
        //通过axios请求API地址
       axios({
            url:'https://www.easy-mock.com/mock/5cd263dad03b596a47cac365/example/banner',
            methods:"get"
        }).then((response)=>{
            this.bannerArr=response.data.data.bannerArr;
